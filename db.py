# db.py — SQLite 版本（GitHub Actions 雲端用）v6
import sqlite3
import pandas as pd
...
def _migrate_daily_score(conn):
    """
    daily_score 資料表欄位遷移
    新增 v6 所需的新欄位（不刪除舊資料）
    """
    new_columns = [
        ("flying_score",   "REAL DEFAULT 0"),
        ("advanced_score", "REAL DEFAULT 0"),
        ("chain_score",    "REAL DEFAULT 0"),
        ("industry",       "TEXT DEFAULT ''"),
        ("themes",         "TEXT DEFAULT ''"),
    ]
    for col, col_type in new_columns:
        if not _table_has_column(conn, "daily_score", col):
            try:
                conn.execute(
                    f"ALTER TABLE daily_score "
...
def init_db():
    ...
    with get_conn() as conn:
        ...
        DROP TABLE IF EXISTS margin_balance;
        ...
        CREATE TABLE IF NOT EXISTS broker_chip (
            stock_id  TEXT NOT NULL,
            date      TEXT NOT NULL,
        ...
        # 自動遷移 daily_score 新欄位（不刪舊資料）
        _migrate_daily_score(conn)
