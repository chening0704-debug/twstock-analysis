# main.py — GitHub Actions 主入口（v5，整合飆股基因）
import os
import sys
import logging
import datetime
...
def main():
    logging.info("=" * 50)
    logging.info("台股智能分析系統 v5.0 啟動（含飆股基因）")
    ...
    logging.info(
        f"飆股基因統計｜"
        f"強力訊號：{len(strong_flying)}支｜"
        f"成形中：{len(flying_scan)-len(strong_flying)}支｜"
...
<div style="font-family:Arial;max-width:500px;
  ...
  <div style="background:#fff;padding:24px;
    ...
    <p style="color:#888;font-size:13px">
      資料完整度 {pct}%<br>
      累積滿 5 個交易日後自動開始產出：<br>
      十大推薦 + 飆股基因分析 + 庫存操作建議<br>
      明日繼續累積，請耐心等待 🙏
    </p>
