# engine/scorer.py — 主分析引擎 v6（整合全部模組）
import logging
import pickle
import pandas as pd
import numpy as np
...
from engine.advanced_technical import compute_advanced_score
from engine.industry_chain   import (
    compute_full_chain_analysis,
    get_international_linkage,
    get_fed_cycle_score,
)
...
def classify_signal(
    ...
    # 飆股強力訊號加成
    if flying_score >= 75 and total >= 65:
        return "BUY"
...
def run_analysis_engine() -> dict:
    ...
    adapt_w     = sl_engine.get_current_weights()
    rule_weights = {
        "fundamental": adapt_w.get(
            "fundamental", DEFAULT_RULE_WEIGHTS["fundamental"]),
        "technical":   adapt_w.get(
            "technical",   DEFAULT_RULE_WEIGHTS["technical"]),
        "chip":        adapt_w.get(
            "chip",        DEFAULT_RULE_WEIGHTS["chip"]),
        "macro":       adapt_w.get(
            "macro",       DEFAULT_RULE_WEIGHTS["macro"]),
        "news":        adapt_w.get(
            "news",        DEFAULT_RULE_WEIGHTS["news"]),
        "flying":      adapt_w.get(
            "flying",      DEFAULT_RULE_WEIGHTS["flying"]),
        "advanced":    adapt_w.get(
            "advanced",    DEFAULT_RULE_WEIGHTS["advanced"]),
    }
    logging.info(f"[Scorer] 自適應權重：{rule_weights}")
    ...
    # 一次性取得全市場共用資料（避免重複呼叫）
    logging.info("取得國際連動與 Fed 週期資料...")
    intl_linkage = get_international_linkage()
    fed_score    = get_fed_cycle_score()
    logging.info(
        f"國際AI情緒：{intl_linkage.get('ai_sentiment','—')} "
        f"Fed分數：{fed_score}"
    )
    ...
    for i, sid in enumerate(stocks):
        try:
            ...
            # 進階技術指標
            adv_result = compute_advanced_score(sid)
            adv_score  = float(
                adv_result.get("advanced_score", 0)
            )

            # 產業鏈分析
            chain_result = compute_full_chain_analysis(
                sid, intl_linkage, fed_score
            )
            chain_score = float(
                chain_result.get("total_chain", 0)
            )
            ...
            score_rule = (
                ...
                + flying_score * rule_weights["flying"]
                + adv_score    * rule_weights["advanced"]
            )

            # 產業鏈加成（最多 +5 分）
            score_rule = min(
                score_rule + chain_score * 0.05, 105
            )
            ...
            if use_rf and rf is not None:
                try:
                    from engine.rf_features import (
            ...
            results.append({
                ...
                "industry":          chain_result.get(
                    "industry","其他"),
                "themes":            ",".join(
                    chain_result.get("themes",[])),
                "signal":            classify_signal(
                    score_total, st, sc_score,
                    rf_prob_up, flying_score),
    ...
    flying_scan = sorted(
        [r for r in flying_all
         if r["stage"] != 4 and r["flying_score"] >= 45],
    ...
    # 自我學習流程
    learning_result = sl_engine.run_daily(top10)

    logging.info(
        f"分析完成｜第一名：{top10.iloc[0]['stock_id']} "
        f"（{top10.iloc[0]['score_total']:.1f}分）"
        f"｜飆股：{len(flying_scan)}支"
        f"｜自適應權重已更新"
    )
